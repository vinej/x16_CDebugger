...
.label SPRITE_VRAM = $13000            // the KERNAL's sprite image area
.label SPRITE_SIZE = 16                // 16x16 pixels
...
.label BOX_X = 304                     // column 38
.label BOX_Y = 200                     // row 25
.label BOX_W = 80                      // 10 cells
.label BOX_H = 80                      // 10 cells

.label BOX_COL = BOX_X / 8
.label BOX_ROW = BOX_Y / 8
.label BOX_COLS = BOX_W / 8
.label BOX_ROWS = BOX_H / 8

.label BOX_CHAR = $A0                  // screen code: reverse space, a solid cell
.label BOX_ATTR = $0E                  // light blue on black

// --- audio -----------------------------------------------------------
// PSG voice 0 plays the bounce blip.
//   freq_word = Hz / (25000000/512 / 2((17) >> 16)) = Hz * 2.684
//   880 Hz (A5) -> 2362 = $093A
.label BLIP_VOICE = 0
.label BLIP_FREQ = 2362
.label BLIP_FRAMES = 15                // envelope length; volume = frames * 4

// YM2151 channel 0 plays a note while the sprite is inside the box.
.label FM_CHANNEL = 0
.label FM_PATCH = 1                  // a ROM instrument patch
.label FM_NOTE = $44                // (octave 4 << 4) | note 4, packed as FMNOTE
...
init_audio:
    ...
    jsr ym_init
    bcs init_audio__no_fm                  // no YM present: run silently
...
move_sprite:
    ...
    bmi move_sprite__vx_neg
    lda #$00
    bra move_sprite__vx_hi
move_sprite__vx_neg:
    lda #$FF
move_sprite__vx_hi:
    adc pos_x+2
    ...
    lda pos_x+2
    bmi move_sprite__clamp_x_lo             // high byte bit 7 set: gone negative
    lda pos_x+1                 // pixel >= PLAY_W - SPRITE_SIZE ?
    cmp #<(PLAY_W - SPRITE_SIZE)
    lda pos_x+2
    sbc #>(PLAY_W - SPRITE_SIZE)
    bcc move_sprite__y                      // still inside
...
move_sprite__y:
    ...
    sta pos_y+1
    lda vel_y+1
    bmi move_sprite__vy_neg
    lda #$00
    bra move_sprite__vy_hi
move_sprite__vy_neg:
    lda #$FF
move_sprite__vy_hi:
    adc pos_y+2
    ...
    lda pos_y+2
    bmi move_sprite__clamp_y_lo
    lda pos_y+1
    cmp #<(PLAY_H - SPRITE_SIZE)
    lda pos_y+2
    sbc #>(PLAY_H - SPRITE_SIZE)
    bcc move_sprite__done                   // still inside
    ...
    stz pos_y
    jsr negate_vy
    bra move_sprite__done
move_sprite__clamp_y_lo:
    stz pos_y
    stz pos_y+1
    stz pos_y+2
    jsr negate_vy
...
check_collision:
    ...
    lda hit
    beq check_collision__miss
    lda #<msg_hit
    ldx #>msg_hit
    bra check_collision__print
check_collision__miss:
    lda #<msg_miss
    ldx #>msg_miss
check_collision__print:
    jsr screen_puts
...
update_fm_note:
    lda hit
    cmp hit_prev
    beq update_fm_note__done                   // no change: leave the note alone
    sta hit_prev
